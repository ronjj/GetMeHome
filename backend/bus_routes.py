...
# OurBus Min
def get_our_bus_min(date,dep_loc,arr_loc, min):
    our_trips = []
    proper_date = format_date(date=date, bus_service="our")
    ...
    for index in range(len(loaded_data)):
        try:
            ...
            newTrip = Trip(ticket_link=api_and_ticket_link, random_num=random_num,date=trip_date, price=price, arr_time=arr_time_12h, arr_location=arr_location, dep_time=dep_time_12h, dep_location=departure_location, bus_serivce=bus, non_stop=non_stop)
            if datetime.strptime(departure_time, "%I:%M%p") >= datetime.strptime(min, "%I:%M%p"):
                trips.append(newTrip)
                our_trips.append(newTrip)
            our_trips.sort(key=lambda x: x.price)
        except:
...
# MegaBus Min
def get_mega_bus_min(date, dep_loc, arr_loc, min):
    mega_trips = []
    mega_location_id = {
        "511":"Ithaca",
        "ithaca":"511",
...
# FlixBus Min
def get_flix_bus_min(date, dep_loc, arr_loc, min):
    flix_trips = []
    flix_location_id = {
        "ithaca": "99c4f86c-3ecb-11ea-8017-02437075395e",
        "new_york": "c0a47c54-53ea-46dc-984b-b764fc0b2fa9",
...
def get_all_min_time(date, dep_loc, arr_loc, min_time):
    ...
    print(f"length of trips after: {len(trips2)}")

    # print(f"Total Options: {len(trips)}")
    # print(f"Cheapest Trip: {trips[0]}")

    return jsonpickle.encode(trips2)
