...
async def get_flix_bus(date, dep_loc, arr_loc, all_or_single):
    async with aiohttp.ClientSession() as session:
        if dep_loc not in constants.FLIX_LOCATION_IDS or arr_loc not in constants.FLIX_LOCATION_IDS:
            return trips_and_discount_response(trips=[], discount_code=[])
        ...
        async def get_intermediate_stations(uid, transfer_type):
            if transfer_type == "Direct":
                uid_string_replace_colons = uid.replace(":", "%3A")
                ...
                async with session.get(intermediate_stations_link) as intermediate_request:
                    intermediate_response = await intermediate_request.text()
                    intermediate_response_json = json.loads(intermediate_response)
                    intermediate_stations_info = intermediate_response_json["itinerary"][0]["segments"]
                    intermediate_count = len(intermediate_stations_info)

                    arrival_longitude = intermediate_response_json['arrival']['coordinates']['longitude']
                    arrival_latitude = intermediate_response_json['arrival']['coordinates']['latitude']
                    arrival_coords = {
                        "longitude": arrival_longitude,
                        "latitude": arrival_latitude
                    }

                    dep_long = intermediate_response_json['departure']['coordinates']['longitude']
                    dep_lat = intermediate_response_json['departure']['coordinates']['latitude']
                    dep_coords = {
                        "longitude": dep_long,
                        "latitude": dep_lat
                    }

                    intermediate_stations_names = [f"{index + 1}. {station['name']}" for index, station in enumerate(intermediate_stations_info)]
                    return intermediate_stations_names, intermediate_count, arrival_coords, dep_coords
            else:
                uid_string_replace_colons = uid.replace(":", "%3A")
                uid_string_final = uid_string_replace_colons.replace("/", "%2F")
                intermediate_stations_link = f"https://global.api.flixbus.com/search/service/v2/trip/details?locale=en_US&trip={uid_string_final}"
                async with session.get(intermediate_stations_link) as intermediate_request:
                    ...
                    for item in intermediate_response_json['itinerary']:
                        count = 1
                        if item['type'] == "ride":
                            for segment in item['segments']:
        ...
        for uid, trip_info in flix_info.items():
            status = trip_info['status']
            transfer_type = trip_info['transfer_type']

            non_stop = "True" if transfer_type == "Direct" else "False"

            if status != 'available':
                continue

            departure_string = trip_info['departure']['date'].split("T")
            try:
                departure_city = constants.FLIX_LOCATION_IDS[trip_info['departure']['station_id']]
            except KeyError:
            ...
            arrival_string = trip_info['arrival']['date'].split("T")
            try:
                arrival_city = constants.FLIX_LOCATION_IDS[trip_info['arrival']['station_id']]
            except KeyError:
            ...
            price = trip_info['price']['total']
            random_num = randrange(10000)

            intermediate_stations_names, intermediate_count, arrival_coords, dep_coords = await get_intermediate_stations(uid, transfer_type)
